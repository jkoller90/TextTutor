...
//has to be put into formSubmission? 
var submissionPromise = new RSVP.Promise(
	function (resolve, reject) {
		var submissionFn = displayForm(res);
		if(displayForm(res))resolve();
		else reject();	
});
...
function formSubmission(req, res) {
 ...
 var submissionPromise = new RSVP.Promise(
		function(fufill, reject){
			var promiseCheck = formSubmission(req,res);
			if(promiseCheck)
				 resolve(promiseCheck);
			else 
				reject(promiseCheck);		
		});

	function promisedSubmission(res){
		// Setting up Form
		var values = [];
		var fields = [];
		var form = new formidable.IncomingForm();
	...
	submissionPromise.then(displayForm(res)); 
	
	//add second parameter to delegate which result screen displays 
}
// ================================================================
// Twillio Messages
// ================================================================
